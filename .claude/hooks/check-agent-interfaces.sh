...
# PostToolUse(Bash) — blocks (exit 1) on confirmed mismatches
...
IS_GIT_OP=false
echo "$COMMAND" | grep -qE 'git (merge|commit|cherry-pick|rebase|checkout|pull)' && IS_GIT_OP=true
...
CHANGED_FILES=$(cd "$REPO_DIR" && git diff --name-only HEAD~1 HEAD 2>/dev/null | grep -E '\.(js|ts|jsx|tsx|swift)$' || true)

if [ -z "$CHANGED_FILES" ]; then
  exit 0
fi

# For Swift projects, check for import mismatches is less critical
# but we still run the JS/TS checks if those files exist
EXPORTS_TMP=$(mktemp)
IMPORTS_TMP=$(mktemp)
...
for f in $CHANGED_FILES; do
  ...
  echo "$f" | grep -qE '\.(js|ts|jsx|tsx)$' || continue
...
if [ -f "$IMPORTS_TMP" ] && [ -s "$IMPORTS_TMP" ]; then
  while IFS=: read -r src_file line_num imp_name req_path; do
    ...
    for candidate in "$SRC_DIR/$req_path.js" "$SRC_DIR/$req_path/index.js" "$SRC_DIR/$req_path"; do
      if [ -f "$candidate" ]; then
        RESOLVED=$(cd "$REPO_DIR" && echo "$candidate" | sed "s|^$REPO_DIR/||")
        break
      fi
    done
    [ -z "$RESOLVED" ] && continue
